      ...
      if (this.state.output.status === 'success') {
        return (
          <Card body outline color="success" className="mt-2">
            <CardText>{this.state.output.result}</CardText>
          </Card>
        )
      } else {
        return (
          <Card body outline color="danger" className="mt-2">
            <CardText>{nl2br(this.state.output.result)}</CardText>
          </Card>
        )
      }
    }
    ...
    return (
      <div className="editor-frame">
        <div className="editor-buttons">
          <Button color="link" className="text-muted" onClick={this.onRunClick}>
            Run
          </Button>
        </div>
        <div>
